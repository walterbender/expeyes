...
Last modified by Walter Bender, 23-Aug-2012 (pep8 clean up)

The micro-controller pins used are mapped into 13 I/O channels
(numbered 0 to 12) and act like a kind of logical channels.  The
Python function calls refer to them using the corresponding number,
ie 0 => A0.
 ...
 * 5 : SEN, Simial to A3 & A4, but has a 5K external pullup resistor
       (Comp input)
 * 6 : SQR1-read, Input wired to SQR1 output
 * 7 : SQR2-read,  Input wired to SQR2 output
 * 8 : SQR1 control, 0 to 5V programmable Squarewave.
       Setting Freq = 0 means 5V, Freq = -1 means 0V
...
SETSQRS    = 203  # Set both square waves, with specified phase difference.
                  # scale, ocr, diff
...
linux_list = ['/dev/ttyACM0', '/dev/ttyACM1', '/dev/ttyACM2', '/dev/ttyACM3',
              '/dev/ttyAMA0']


def open(dev=None):
    '''
    If EYES hardware in found, returns an instance of 'Eyes', else
    returns None.
    '''
    obj = Eyesjun()
    if obj.fd is not None:
...
#-------------------------------------- CTMU -------------
    ...
    def irsend1(self, d1):
        ...
        res = self.fd.read(1)
        if res != 'D':
            self.msg = 'IRSEND1 ERROR ' + res
            print 'IRSEND1 ERROR', res
            return
        return 1
    ...
    def irsend4(self, d1, d2, d3, d4):
        ...
        res = self.fd.read(1)
        if res != 'D':
            self.msg = 'IRSEND4 ERROR ' + res
            print 'IRSEND4 ERROR', res
            return
        return 1

    def nano_delay(self, i):
        '''
        Using the CTMU of PIC, measure r2r from IN2 or SEN. uses cap
        of IN1. Incomplete
        ch = 3
    ...
    def measure_cv(self, ch, ctime, i=5.5):
        '''
        Using the CTMU of PIC, charges a capacitor connected to IN1,
        IN2 or SEN, for 'ctime' microseconds and then mesures the
        voltage across it.  The value of current can be set to .55uA,
        5.5 uA, 55uA or 550 uA
        '''
        if i > 500:   # 550 uA
            irange = 0
        elif i > 50:  # 55 uA
        ...
        res = self.fd.read(1)
        if res != 'D':
            self.msg = 'MEASURECV ERROR ' + res
            print 'MEASURECV ERROR', res
            return
        res = self.fd.read(2)
        iv = ord(res[0]) | (ord(res[1]) << 8)
        v = self.m12[ch] * iv + self.c[ch]
    ...
    def measure_cap(self, ctmin=10):
        ...
        value. Measurement is done by charging the capacitor with 5.5
        uA for a given time interval.
        '''
    ...
    def set_current(self, ch, i):  # channel 3 or 4, 0 means stop CTMU
        '''
        Sets CTMU current 'i' on a channel 'ch' and returns the
        voltage measured across the load. Allowed values of current
        are .55, 5.5, 55 and 550 micro ampleres.
        '''
        if i > 500:   # 550 uA
            irange = 0
        elif i > 50:  # 55 uA
        ...
        res = self.fd.read(1)
        if res != 'D':
            self.msg = 'SETCURRENT ERROR' + res
            print 'SETCURRENT ERROR', res
            return
        res = self.fd.read(2)
        iv = ord(res[0]) | (ord(res[1]) << 8)
        v = self.m12[ch] * iv + self.c[ch]
    ...
    def read_temp(self):
        ...
        regulator with temeperature.
        '''
        self.sendByte(READTEMP)
        res = self.fd.read(1)
        if res != 'D':
            self.msg = 'READTEMP error' + res
    ...
    def tim_helper(self, cmd, src, dst):
        ...
        if cmd == MULTIR2R:
            if src not in [0, 3, 4, 5, 6, 7]:
                ...
                self.msg = 'Pin should be digital input capable: \
0, 3, 4, 5, 6 or 7'
                return -1
            if dst > 249:
        ...
        if cmd in [R2RTIME, R2FTIME, F2RTIME, F2FTIME]:
            if src not in [0, 3, 4, 5, 6, 7] or dst not in [0, 3, 4, 5, 6, 7]:
                self.msg = 'Both pins should be digital input capable: \
0, 3, 4, 5, 6 or 7'
                print 'Both pins should be digital input capable: \
0, 3, 4, 5, 6 or 7'
                return -1
        if cmd in [SET2RTIME, SET2FTIME, CLR2RTIME, CLR2FTIME, HTPUL2RTIME,
                   HTPUL2FTIME, LTPUL2RTIME, LTPUL2FTIME]:
            if src not in [8, 9, 10, 11]:
                self.msg = 'Starting pin should be digital output capable: \
8, 9, 10 or 11'
                print 'Starting pin should be digital output capable: \
8, 9, 10 or 11'
                return -1
            if dst not in [0, 3, 4, 5, 6, 7]:
                self.msg = 'Destination pin should be digital input capable: \
0, 3, 4, 5, 6 or 7'
                print 'Destination pin should be digital input capable: \
0, 3, 4, 5, 6 or 7'
                return -1
        self.sendByte(cmd)
        self.sendByte(src)
        ...
        res = self.fd.read(1)
        data = self.fd.read(4)
        raw = struct.unpack('I' * 1, data)  # 32 bit data from T4/T5 counter,
                                            # 0.125us cycles
...
# -------------------- Passive Time Interval Measurements --------------------
    ...
    def r2rtime(self, pin1, pin2):
        '''
        Time between two rising edges. The pins must be distinct. For
        same pin, use multi_r2rtime
        '''
        return self.tim_helper(R2RTIME, pin1, pin2)

    def f2ftime(self, pin1, pin2):
        '''
        Time between two falling edges. The pins must be distinct. For
        same pin, use multi_r2rtime
        '''
    ...
    def r2ftime(self, pin1, pin2):
        '''
        Time between a rising edge to a falling edge. The pins could
        be same or distinct.
        '''
    ...
    def f2rtime(self, pin1, pin2):
        '''
        Time between a falling edge to a rising edge. The pins could
        be same or distinct.
        '''
    ...
    def multi_r2rtime(self, pin, skip=0):
        '''
        Time between rising edges, could skip desired number of edges
        in between. (pin, 9) will give time required for 10 cycles of
        a squarewave, increases resolution.
        '''
        return self.tim_helper(MULTIR2R, pin, skip)

    def get_frequency(self, pin):
        '''
        This function measures the frequency of an external 0 to 5V
        PULSE on digital inputs, by calling multi_r2rtime().
        '''
...
#---------------- Square Wave Generation & Measuring the Frequency -----------
    ...
    def set_pwm(self, osc, dc, resol=14):
        # osc and duty cycle, resolution 14 bits by default
        '''
        Sets PWM on SQR1 / SQR2. The frequency is decided by the
        resolution in bits.
        '''
        if resol < 4 or resol > 16 or dc < 0 or dc > 100:
            return
        ocxrs = 2 ** resol
        ocx = int(0.01 * dc * ocxrs + 0.5)
        # print ocxrs, ocx
    ...
    def set_sqr1_pwm(self, dc, resol=14):
        # Duty cycle, resolution 14 bits (f = 488Hz) by default
        '''
        Sets 488 Hz PWM on SQR1. Duty cycle is specified in
        percentage. The third argument, PWM resolution, is 14 bits by
        default. Decreasing this by one doubles the frequency.
        '''
        return self.set_pwm(0, dc, resol)

    def set_sqr2_pwm(self, dc, resol=14):
        '''
        Sets 488 Hz PWM on SQR2. Duty cycle is specified in
        percentage. The third argument, PWM resolution, is 14 bits by
        default. Decreasing this by one doubles the frequency.
        '''
    ...
    def set_sqr1_dc(self, volt):
        '''
        PWM DAC on SQR1. Resolution is 10 bits (f = 7.8 kHz) by
        default. External Filter is required to get the DC The voltage
        can be set from 0 to 5 volts.
        '''
        # 100% means 5 volts, 10 bit resolution, 8kHz
        return self.set_pwm(0, volt * 20.0, 10) / 20

    def set_sqr2_dc(self, volt):
        '''
        PWM DAC on SQR2. Resolution is 10 bits (f = 7.8 kHz) by
        default. External Filter is required to get the DC The voltage
        can be set from 0 to 5 volts.
        '''
    ...
    def set_osc(self, chan, freq):  # Freq in Hertz, osc 1 or 2
        ...
        if freq < 0:  # Disable Timer and Set Output LOW
            ...
            # Possible Timer period values
            mtvals = [T, T * 8, T * 64, T * 256]
    ...
    def set_sqr1(self, freq):
        '''
        Sets the frequency of SQR1 (between .7Hz and 200kHz). All
        intermediate values are not possible.  Returns the actual
        value set.
        '''
    ...
    def set_sqr2(self, freq):
        '''
        Sets the frequency of SQR2 (between .7Hz and 200kHz). All
        intermediate values are not possible.  Returns the actual
        value set.
        '''
        return self.set_osc(9, freq)

    def set_sqrs(self, freq, diff=0):
        # Freq in Hertz, phase difference in % of T
        '''
        Sets the output frequency of both SQR1 & SQR2. The function
        returns actual value set. The second argument is the phase
        difference between them in percentage.
        '''
...
# --------------------------------- ADC & DAC -------------------------------
    ...
    def set_voltage(self, v):
        '''
        Sets the PVS output. range is from -5 to + 5 volts. Reads the
        actual value to apply correction.  Returns the voltage
        readback of the voltage at PVS.
        '''
        if v < 0 or v > 5.0:
    ...
    def get_voltage_within(self, ch, vmax):
        # Channel and the expected maximum value, < 5V
        '''
        Sets the DAC to vmax and uses it as external +Vref, to
        increase resolution
        '''
    ...
    def capture(self, ch, ns, tg):  # datasize is 1 byte
        '''
        Arguments : channel number , number of samples and timegap
        between consecutive digitizations. Returns two lists of size
        'ns': time and volatge.  Each item is 1 bytes size, truncated
        ADC data.
        '''
    ...
    def capture_hr(self, ch, ns, tg):  # datasize is 2 byte
        ...
        Arguments : channel number , number of samples and timegap
        between consecutive digitizations. Returns two lists of size
        'ns': time and volatge.  Each item is 2 bytes size, holding 12
        bit ADC data.
        '''
        if tg < 4:
            self.msg = 'Minimum Timegap is 4 us'
    ...
    def capture2(self, cha, chb, ns, tg):  # 2 channels, datasize is 1 byte
        '''
        Arguments : 2 channel numbers , number of samples and timegap
        between consecutive digitizations. Returns two lists of size
        'ns': time and volatge.  returns 4 lists. Time & voltages, 1
        byte ADC data
        '''
        if tg < 8:
    ...
    def capture2_hr(self, cha, chb, ns, tg):  # 2 channels, datasize is 2 byte
        '''
        Arguments : 2 channel numbers , number of samples and timegap
        between consecutive digitizations. Returns 4 lists of size
        'ns': times and volatges.  Each item is 2 bytes size, holding
        12 bit ADC data.
        '''
        if tg < 8:
            self.msg = 'Minimum Timegap is (4*number of channels)usec'
    ...
    def capture3(self, ch1, ch2, ch3, ns, tg):
        # 3 channels, datasize is 2 byte
        '''
        Arguments : 3 channel numbers, number of samples and timegap
        between consecutive digitizations. Returns two lists of size
        'ns': time and volatge.
        '''
        if tg < 12:
    ...
    def capture4(self, ch1, ch2, ch3, ch4, ns, tg):
        # 4 channels, datasize is 1 byte
        '''
        Arguments : 4 channel numbers , number of samples and timegap
        between consecutive digitizations. Returns two lists of size
        'ns': time and volatge.
        '''
        if tg < 16:
...
#------------------- Modifiers for Capture ------------------------------
    ...
    def disable_actions(self):
        '''
        Disable all modifiers to the capture call. The capture calls
        will be set to do analog triggering on the first channel
        captured.
        '''
        self.sendByte(SETACTION)
        self.sendByte(AANATRIG)
        # Self trigger on channel zero means the first channel captured
        self.sendByte(0)
        res = self.fd.read(1)
        if res != 'D':
    ...
    def enable_action(self, action, ch):
        ...
        if res != 'D':
            ...
            print 'SETACTION ERR: action = %d ch = %d' % (action, ch), res
            return
        return action
    ...
    def enable_wait_rising(self, ch):
        '''
        Wait for a rising EDGE on the speciied 'pin' just before every
        Capture.
        '''
    ...
    def enable_wait_falling(self, ch):
        '''
        Wait for a falling EDGE on the speciied 'pin' just before
        every Capture.
        '''
    ...
    def enable_pulse_low(self, ch):
        '''
        Generate a LOW TRUE Pulse on the speciied 'pin', just before
        every Capture.
        '''
